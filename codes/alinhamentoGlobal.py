...
def miss(A,B):
    ...
    missesB = []
    for element in range(0, i):
        if(A[element]!=B[element] and (A[element]!='-'or B[element]!='-')):
            missesA.append(A[element])
            missesB.append(B[element])
...
def align(A,B):
    ...
    return AlignmentA,AlignmentB,finalScore(AlignmentA,AlignmentB),miss(AlignmentA,AlignmentB)

pala = 'abracadabra'
palb = 'apracadapra'
print(align(pala,palb))
print(pala)